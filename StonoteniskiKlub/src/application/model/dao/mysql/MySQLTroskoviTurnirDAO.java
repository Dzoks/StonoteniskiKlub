...
public class MySQLTroskoviTurnirDAO implements TroskoviTurnirDAO {
	...
	public ObservableList<TroskoviTurnirDTO> SELECT_ALL() {
		...
		try {
			...
			while (rs.next()) {
				TurnirDTO turnir = new TurnirDTO(rs.getInt("turnirId"), rs.getString("naziv"),
						rs.getDate("turnirDatum"), false);
				listaTroskoviTurnir.add(new TroskoviTurnirDTO(rs.getInt("Id"), rs.getDate("Datum"),
	...
	public boolean INSERT(TroskoviTurnirDTO troskovi, TurnirDTO turnir) { // radi,
																			// vidjeti
																			// povratnu
																			// vrijednost
