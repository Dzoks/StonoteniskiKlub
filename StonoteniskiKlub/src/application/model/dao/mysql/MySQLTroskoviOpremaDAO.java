...
public class MySQLTroskoviOpremaDAO implements TroskoviOpremaDAO{
	...
	public  ObservableList<TroskoviOpremaDTO> SELECT_ALL() {
		...
		try {
			...
			while(rs.next()) {
				Narudzba narudzba = new Narudzba(rs.getInt("Id"), rs.getDate("nDatum"), true, true, rs.getInt("DISTRIBUTER_OPREME_Id"));
				listaTroskoviOprema.add(new TroskoviOpremaDTO(rs.getInt("TRANSAKCIJA_Id"), rs.getDate("Datum"), rs.getDouble("Iznos"), rs.getString("Opis"),rs.getString("Tip"),narudzba));
			}
	...
	public  boolean INSERT(TroskoviOpremaDTO troskovi, Narudzba narudzba) { //radi, vidjeti povratnu vrijednost
