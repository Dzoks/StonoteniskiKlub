...
public class MySQLNarudzbaStavkaDAO implements NarudzbaStavkaDAO {
	...
	public ObservableList<NarudzbaStavka> SELECT_BY_IDNARUDZBE(Integer id) {
		...
		try {
			...
			while (rs.next()) {
				listaStavkiNarudzbe.add(new NarudzbaStavka(rs.getInt("NARUDZBA_Id"), rs.getInt("OPREMA_TIP_Id"),
						rs.getString("Velicina"), rs.getInt("Kolicina"), rs.getDouble("Cijena"),
	...
	public void INSERT(NarudzbaStavka narudzbaStavka) {
		...
		try {
			c = ConnectionPool.getInstance().checkOut();
			ps = ConnectionPool.prepareStatement(c, SQL_INSERT, false, narudzbaStavka.getIdNarudzbe(),
					narudzbaStavka.getIdTipaOpreme(), narudzbaStavka.getVelicina(), narudzbaStavka.getKolicina(),
	...
	public void DELETE(NarudzbaStavka narudzbaStavka) {
		...
		try {
			c = ConnectionPool.getInstance().checkOut();
			ps = ConnectionPool.prepareStatement(c, SQL_DELETE, false, narudzbaStavka.getIdNarudzbe(),
	...
	public void UPDATE_OBRADJENO(NarudzbaStavka stavkaNarudzbe) {
		...
		try {
			c = ConnectionPool.getInstance().checkOut();
			ps = ConnectionPool.prepareStatement(c, SQL_UPDATE_OBRADJENO, false, stavkaNarudzbe.getIdNarudzbe(),
