...
public class MySQLTimDAO implements TimDAO {
	...
	public ObservableList<UcesnikPrijavaDTO> getDouble(Integer idTurnira, Integer idKategorije) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				retVal.add(new UcesnikPrijavaDTO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
						rs.getString(5).charAt(0), rs.getDate(6), rs.getInt(13), idTurnira, idKategorije,
						rs.getDate(15)));
				retVal.add(new UcesnikPrijavaDTO(rs.getInt(7), rs.getString(8), rs.getString(9), rs.getString(10),
						rs.getString(11).charAt(0), rs.getDate(12), rs.getInt(14), idTurnira, idKategorije,
						rs.getDate(16)));
			}
		} catch (SQLException e) {
			e.printStackTrace();
