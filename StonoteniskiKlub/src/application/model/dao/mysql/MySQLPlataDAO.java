...
public class MySQLPlataDAO implements PlataDAO {
	...
	public ObservableList<PlataDTO> SELECT_ALL() {
		...
		try {
			...
			while (rs.next()) {
				ZaposleniDTO zaposleni = new ZaposleniDTO(rs.getInt("OSOBA_Id"), rs.getString("Ime"),
						rs.getString("Prezime"), rs.getString("ImeRoditelja"), rs.getString("JMB"),
						rs.getString("Pol").charAt(0), rs.getDate("DatumRodjenja"), rs.getBlob("Fotografija"),
						DAOFactory.getDAOFactory().getOsobaDAO().getTelefoni(rs.getInt("Id")), rs.getBoolean("Aktivan"),
						DAOFactory.getDAOFactory().getZaposlenjeDAO().selectAllById(rs.getInt("OSOBA_Id")));
				listaPlata.add(new PlataDTO(rs.getInt("Id"), rs.getDate("Datum"), rs.getDouble("Iznos"),
