...
public class OpremaDAO {
	...
	private final static String SQL_UPDATE = "UPDATE oprema SET OPREMA_TIP_Id=? WHERE Id=?";
	private final static String SQL_UPDATE_OBRISAN = "UPDATE oprema SET Obrisan=true WHERE Id=?";
	
	public static void UPDATE(Oprema oprema, Integer idOpreme) {
		Connection c = null;
		PreparedStatement ps = null;
		
		try {
			c = ConnectionPool.getInstance().checkOut();
			ps = ConnectionPool.prepareStatement(c, SQL_UPDATE, false, idOpreme, oprema.getId());
			ps.executeUpdate();
